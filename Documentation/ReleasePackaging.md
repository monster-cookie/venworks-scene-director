...
- Spriggit YAML or plugin changes: assemble the plugin locally and commit the updated `Staging/Venworks-DynamicScenesEngine.esm`.
...
`Staging/Venworks-DynamicScenesEngine.esp` is an editing artifact. It may be committed for local workflows, but it is intentionally excluded from release zips.
...
Each package includes `Staging/Venworks-DynamicScenesEngine.esm` and the platform archives for that package:
...
2. Confirm `Staging/Venworks-DynamicScenesEngine.esm` is current.
3. Confirm all required PC and XBox `.ba2` archives are present in `Staging`.
4. Confirm `Staging/Venworks-DynamicScenesEngine.esp` is not needed by any release package.
...
The Nexus upload uses `Venworks - Dynamic Scenes Engine` as the display name, `main` as the category, and enables archive replacement and mod manager downloads.
